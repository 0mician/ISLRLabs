...
#vignette("cancerdata")
par(mfrow=c(1,1))
load(file="../reports/GeneExpressionVijver/VIJVER.Rdata")
set.seed(5)
dim(data)
...
pcr.pred <- predict(pcr.mod,data[test,], ncomp=33,type="response")
pred=rep("DM",length(meta[test]))
hist(pcr.pred)#need pls-glm for logistic regression!
